...
export default function OrderTrack() {
  ...
  useEffect(() => {
    ...
    const handleOrderUpdate = (updatedOrder: Order) => {
      console.log("📦 Order update received:", updatedOrder.orderId);
      setOrder((currentOrder) => {
        if (currentOrder && updatedOrder.orderId === currentOrder.orderId) {
          return updatedOrder;
        }
        return currentOrder;
      });
    ...
    const handleOrderCancelled = (cancelledOrder: Order) => {
      ...
      setOrder((currentOrder) => {
        if (currentOrder && cancelledOrder.orderId === currentOrder.orderId) {
  ...
  // Track order when order changes
  useEffect(() => {
    const socket = socketRef.current;

    if (order && socket) {
      console.log("📡 Joining tracking room:", order.orderId);
      socket.emit("track-order", order.orderId);

      return () => {
        console.log("🛑 Leaving tracking room:", order.orderId);
        socket.emit("stop-tracking", order.orderId);
      };
    }
  }, [order]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        >
          ...
          <p className="text-gray-400">Enter your order ID to track status</p>
        </motion.div>
        ...
        >
          <div className="flex gap-2">
            <input
              ...
              onChange={(e) => setOrderId(e.target.value.toUpperCase())}
              placeholder="Enter Order ID (e.g., ORD-ABC12345)"
              className="flex-1 bg-gray-900/80 border border-orange-500/30 rounded-xl px-4 py-4 text-white placeholder-gray-500 focus:border-orange-500 focus:outline-none"
            />
            <button
              type="submit"
              disabled={loading}
              className="px-6 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-xl font-semibold hover:shadow-lg hover:shadow-orange-500/50 transition-all disabled:opacity-50 flex items-center gap-2"
            >
              {loading ? (
                <Loader2 className="animate-spin" size={20} />
              ) : (
        ...
        {error && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="bg-red-500/20 border border-red-500/30 text-red-300 px-4 py-3 rounded-lg mb-4 text-center"
          >
            {error}
          </motion.div>
        )}
        ...
        {order && (
          ...
          >
            {/* Status Card */}
            <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-6 border-2 border-orange-500/30">
                  ...
                  return (
                    <div className="text-center">
                      <motion.div
                        ...
                        animate={{ scale: 1 }}
                        transition={{ type: "spring" }}
                        className={`inline-block p-4 rounded-full bg-${statusInfo.color.replace(
                          "text-",
                          ""
                        )}/20 mb-3`}
                      >
                        <Icon size={48} className={statusInfo.color} />
                      </motion.div>
            ...
            {/* Order Info */}
            <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-6 border border-orange-500/30">
